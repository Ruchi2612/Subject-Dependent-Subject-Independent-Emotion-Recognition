...
% Date:   11/03/2023
%--------------------------------------------------------------------------
% Code for Subject Independent (for Binary Classification)
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
    ...
    XTrain = load(strcat('Val_TrainFeatures','.mat'));
    YTrain = load(strcat('Val_TrainLabels','.mat'));
    XTest = load(strcat('Val_TestFeatures','.mat'));
    YTest = load(strcat('Val_TestLabels','.mat'));
    
    data = XTrain.trainFeatures;
    label = YTrain.YTraining_val;
    s = size(data,1);
    index = randperm(s);
...
%--------------------------------------------------------------------------
    fprintf('AROUSAL:\n')
    cd('Add feature data path')
    result_path = fullfile('Add feature result path');
    
    XTrain = load(strcat('Aro_TrainFeatures','.mat'));
    YTrain = load(strcat('Aro_TrainLabels','.mat'));
    XTest = load(strcat('Aro_TestFeatures','.mat'));
    YTest = load(strcat('Aro_TestLabels','.mat'));
    
    data = XTrain.trainFeatures;
    label = YTrain.YTraining_aro;
    s = size(data,1);
    index = randperm(s);
    train = index(1:round(s*0.80));
    test = index(round(s*0.80)+1:end);
...
%--------------------------------------------------------------------------
    ...
    XTrain = load(strcat('Dom_TrainFeatures','.mat'));
    YTrain = load(strcat('Dom_TrainLabels','.mat'));
    XTest = load(strcat('Dom_TestFeatures','.mat'));
    YTest = load(strcat('Dom_TestLabels','.mat'));
    
    data = XTrain.trainFeatures;
    label = YTrain.YTraining_dom;
    s = size(data,1);
    index = randperm(s);
    train = index(1:round(s*0.80));
    test = index(round(s*0.80)+1:end);
